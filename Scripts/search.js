...
let imgarr = [
  ...
  let btn = document.getElementById("searchbtn");
       ...
       document.getElementById("search1234").innerText=null;       
    ...
    for (let i = 0; i < 15; i++) {
      run(i, inp);
       ...
       function run(x, inp) {
        ...
        let imag = imgarr[Math.floor(Math.random() * n)];
        let company = comparr[Math.floor(Math.random() * comparr.length)];
        let taga = inp + " " + tagarr[Math.floor(Math.random() * tagarr.length)];
        let pri = Math.floor(Math.random() * 3000);
        let off = Math.round(Math.random() * 0.3 * 100) / 100;
        let mrp = Math.floor(pri / (1 - off));
        let stock = Math.round(Math.random() * 100) / 100;
        let count = x;
        let qty = 1;
        box.innerHTML = `
          <div class="srch_img1">
